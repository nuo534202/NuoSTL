...
public:
        ...
        : _size(nv._size), _capacity(nv._capacity), _alloc(nv._alloc)
    {
        ...
        {
            for (i = 0; i < _size; i++)
                std::allocator_traits<Allocator>::construct(_alloc, _data + i, x[i]);
        }
        catch
        {
            for (size_type j = 0; j < i; j++)
    ...
    constexpr nuo_vector(nuo_vector&& nv, const type_identity_t<Allocator>& alloc)
        : _size(nv._size), _capacity(nv._capacity), _alloc(_alloc)
    {
        if (_alloc == nv._alloc)
        {
            _data = _data;
    ...
    constexpr nuo_vector(initializer_list<T> il, const Allocator& alloc = Allocator())
        : _alloc(alloc)
    {
        _size = il.size();
        _capacity = std::bit_ceil(_size);
        ...
        try
        {
            for (auto it = il.begin(); it != end(); it++, i++)
                std::allocator_traits<Allocator>::construct(_alloc, _data + i, *it);
        }
    ...
    constexpr nuo_vector& operator=(initializer_list<T> il)
        : _alloc(Allocator())
    {
        _size = il.size();
        _capacity = std::bit_ceil(_size);
    ...
    constexpr void assign(InputIter first, InputIter last);
    {
        ...
        if (_capacity < last - first)
        {
            ...
            _capacity = std::bit_ceil(_size);
            _data = std::allocator_traits<Allocator>::allocatoe(_alloc, _capacity);
        }
            ...
            try
            {
                for(auto&& elem : rg)
                {
                    if (_capacity < _size)
                    {
                        ...
                        pointer new_data = std::allocator_traits<Allocator>::allocate(_alloc, _capacity);
